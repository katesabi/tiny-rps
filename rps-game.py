...
def switch_to_start() -> None:
    ...
    # Скрыть остальные страницы
    rules_frame.pack_forget()
    game_frame.pack_forget()


def switch_to_rules() -> None:
...
def build_start_screen() -> None:
    ...
    ttk.Button(
        btn_frame, text="🎮 Play!", style="Sweet.TButton", command=switch_to_game
    ).pack(side="left", padx=15)

    ttk.Button(
        btn_frame, text="❓ How to Play", style="Sweet.TButton", command=switch_to_rules
    ).pack(side="left", padx=15)


def build_rules_screen() -> None:
    ttk.Label(rules_frame, text="How to Play", style="Header.TLabel").pack(pady=(30, 15))
    ...
    ttk.Button(
        btn_frame, text="🔙 Back", style="Sweet.TButton", command=switch_to_start
    ).pack()
...
root.geometry("400x550")  # Чуть выше, чтобы всё помещалось
